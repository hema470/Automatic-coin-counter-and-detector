...
# Force immediate logging and set a writable directory for YOLO settings
...
try:
    ...
    model = YOLO("best.pt")
    print("--- [SYSTEM] YOLOv10 WEIGHTS LOADED ---", file=sys.stderr)
...
def sanchay_ai_engine(image, upi_id, manual_total, savings_split):
    if image is None:
        return None, None, None, None, "⚠️ Please upload a coin photo."

    # --- Step A: Precise YOLO Detection ---
    # iou=0.45 helps prevent overlapping double-detections
    ...
    # Build Denomination Breakdown Table
    denom_data = [[f"₹{k}", v, f"₹{k*v}"] for k, v in sorted(denom_counts.items(), reverse=True)]
    denom_df = pd.DataFrame(denom_data, columns=["Denomination", "Quantity", "Sub-total"])
    ...
    audit_df = pd.DataFrame(audit_data, columns=["Total Coins", "AI Value", "Oxidation", "Audit Status"])

    # Build Savings Ledger
    savings_data = [[
    ...
    final_viz = cv2.cvtColor(annotated_img, cv2.COLOR_BGR2RGB)

    return final_viz, denom_df, audit_df, savings_df, status_msg

# 4. UI DESIGN
# Define theme separately for compatibility
sanchay_theme = gr.themes.Soft(primary_hue="emerald", secondary_hue="blue")

with gr.Blocks(title="Sanchay-AI") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            out_label = gr.Label(label="Transaction Status")
            out_img = gr.Image(label="Computer Vision Analysis")
            
            with gr.Row():
                with gr.Column():
                    gr.Markdown("#### 🔢 Denomination Breakdown")
                    denom_table = gr.Dataframe()
                with gr.Column():
    ...
    btn.click(
        ...
        outputs=[out_img, denom_table, audit_table, savings_table, out_label]
    )

# 5. LAUNCH
if __name__ == "__main__":
    demo.launch(
        share=False,          # Turn this OFF to avoid the 504 error
